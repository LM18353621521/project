...
//        dump($goods_list);die;
    ...
    public function goodsInfo(){
        ...
        $goods_id = I("get.id/d");
        $give_vip_id = I("get.qrid/d");//赠送名额id

        //绑定上下级关系
        if($give_vip_id){
            $user = session('user');
            $user_give_vip = M('user_give_vip')->where(array('id'=>$give_vip_id))->find();
            $result = bind_distribution($user['user_id'],$user_give_vip['user_id']);
        }
        ...
        //订单数量
        $order_count  = M('order')->where(['user_id'=>$user_id])->count();
...
//        dump($goods->toArray());die;
    ...
    public function search(){
        ...
        if($q) $where['goods_name|keywords'] = array('like','%'.$q.'%');
