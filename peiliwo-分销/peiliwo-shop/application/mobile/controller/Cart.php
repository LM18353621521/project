...
//                $where_o['pay_status']=1;
                $has_first_order  = M('order')->where($where_o)->find();

//                $user['discount'] = (empty($user['discount'])) ? 1 : $user['discount'];
                if($has_first_order){
                    $user['discount'] = 1;
                }else{
                    $first_discount = tpCache('shopping.first_discount');
                    $user['discount'] = $first_discount/100;
                }
    ...
    public function cart2(){
        ...
        if(empty($address)){
            header("Location: ".U('Mobile/User/add_address',array('source'=>'cart2','action'=>$action,'goods_id'=>$goods_id,'goods_num'=>$goods_num,'item_id'=>$item_id,'qrid'=>$qrid)));
            exit;
        }else{
            $address['province_name'] = M('region')->where(array('id'=>$address['province']))->getField('name');
            $address['city_name'] = M('region')->where(array('id'=>$address['city']))->getField('name');
            $address['district_name'] = M('region')->where(array('id'=>$address['district']))->getField('name');
        ...
        # 首次购买签署协议
        $agreementUrl = 0;
        $oeder_where['user_id'] = $this->user_id;
        $orderCount = M('order')->where($oeder_where)->count(); // 购买数量
        if ($orderCount==0) { 
            $agreement_where['cat_id'] = 9;
            $agreement = M('article')->where($agreement_where)->find(); //协议的文章
            $agreementUrl = U('Mobile/Article/detail',array('article_id'=>$agreement['article_id']));
        }
        $this->assign('agreementUrl', $agreementUrl);  //协议链接
        # END 8-17
        # 市代或区代的身份产品选择代理地区
        $goods_where['goods_id'] = $goods_id;
        $goods_identity = M('goods')->where($goods_where)->find(); //商品信息
        // 商品关联的系列对应的等级
        $level_identity = Db::name('user_level')
            ->alias('ul')
            ->field('ul.*')
            ->join('goods g','ul.identity_id = g.identity_id','left')
            ->where(array('g.goods_id'=>$goods_identity['goods_id']))
            ->find();
        $identity_id = 0;
        if (!empty($level_identity)) {
            // 市代或者区代
            if ($level_identity['level_id']==5 || $level_identity['level_id']==6) {
                $identity_id = $level_identity['level_id'];
            }
        }
        $this->assign('identity_id', $identity_id); // 身份产品关联的等级
        # END
        // 用户信息 判断用户是否绑定手机号
        $user_info = M('users')->where(array('user_id'=>$this->user_id))->find();
        ...
        //订单数量
        $order_count  = M('order')->where(['user_id'=>$this->user_id])->count();
        if ($order_count == 0 && $cartPriceInfo['goods_num'] > 1) {
            $this->error('首次下单只能购买一件商品', 'Cart/index');
        }
    ...
    public function cart3(){
        ...
        if($_REQUEST['act'] == 'submit_order') {
            ...
            //判断赠送名额是否已被使用
            if($qrid>0){
                $user_give_vip_use  = M('user_give_vip')->where(array('id'=>$qrid))->find();
                if($user_give_vip_use['status']==2){
                    exit(json_encode(array('status'=>-1,'msg'=>'该名额已被使用，不可重复使用','result'=>null)));
                }
                if($user_give_vip_use['status']==4){
                    exit(json_encode(array('status'=>-1,'msg'=>'该名额已被禁用，不可下单','result'=>null)));
                }
            }
            ...
            $agency_area = 0;
            if (!empty($level_identity)) {
                if ($level_identity['level_id']==5) {
                    $agency_area = input("district/d");
                    // 是否有人代理了该区
                    $has_other_agent =  M('agency_area')->where(array('area_id'=>$agency_area))->find();
                    if ($has_other_agent ) {
                        exit(json_encode(array('status'=>-1,'msg'=>'已经有人代理了该地区','result'=>null)));
                    }
                }elseif ($level_identity['level_id']==6) {
                    $agency_area = input("city/d");
                    // 是否有人代理了该市
                    $has_other_agent =  M('agency_area')->where(array('city_id'=>$agency_area))->find();
                    if ($has_other_agent ) {
                        exit(json_encode(array('status'=>-1,'msg'=>'已经有人代理了该城市','result'=>null)));
                    }
                }
            }
            # END
            ...
            $result = $orderLogic->addOrder($this->user_id,$address_id,$shipping_code,$invoice_title,$coupon_id,$car_price,$user_note,$pay_name,$agency_area,$qrid); // 添加订单
            exit(json_encode($result));
        }
    ...
    public function cart4(){
        ...
        $order_goods=db('order_goods')->where(array('order_id'=>$order_id))->select();
        foreach ($order_goods as $value){
            if($value['spec_key']>0){
                $spec_goods_price = db('spec_goods_price')->where(array('spec_key'=>value['spec_key']))->find();
                if(empty($spec_goods_price)){
                    $this->error($value['goods_name'].'（'.$value['spec_key_name'].'）已被下架',U("Mobile/Order/order_detail",array('id'=>$order_id)));
                }
            }
            $goods = db('goods')->where(array('goods_id'=>$value['goods_id']))->find();
            if(empty($goods)||$goods['is_on_sale']==0){
                $this->error($value['goods_name'].'已被下架',U("Mobile/Order/order_detail",array('id'=>$order_id)));
            }
        }
