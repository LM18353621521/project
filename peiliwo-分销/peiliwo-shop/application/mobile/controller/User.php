...
class User extends MobileBase
{
    ...
    */
    ...
    //奖池累加
    public function  jackpot_count(){
        $start_day=15;
        $start = strtotime('2018-11');
        $end = strtotime('2019-2');
    ...
    public function region_prize(){
       ...
       // $where['is_jackpot_prize']=0;
        ...
        //改变可分奖的订单为已分奖
        $res_order =M('order')->alias('a')->where($where)->update(array('is_jackpot_prize'=>0));
    ...
    public function index()
    {
        ...
        $count = M('order')->where("user_id=$user_id AND (pay_status=1 OR pay_code='cod') AND shipping_status !=1 AND order_status in(0,1)")->count();
        $order_count[1] = $count;
        //待收货
        $where['pay_status']=1;
...
//        dump($order_count);die;
    ...
    public function give_quota()
    {
        ...
        $quota = M('user_give_vip')->where($where)->select();

        $this->assign('quota_list', $quota);
        $this->assign('type',$type);
//        $this->assign('page', $data['show']);
        if (input('is_ajax')) {
            return $this->fetch('ajax_coupon_list');
    ...
    public function getQrcode()
    {
        ...
        $id = $_GET['id']; // 赠送名额id
        $qrcode = M('user_give_vip')->where('id',$id)->find(); // 赠送名额相关数据
        $user = M('users')->where('user_id',$qrcode['user_id'])->find();

        $url = request()->domain().U('Goods/goodsInfo',['id'=>$qrcode['goods_id'],'qrid'=>$qrcode['id']]);
        ...
        $urlPath = request()->domain().'/'.$after_path;
        $this->assign('user', $user);
        $this->assign('urlPath', $urlPath);
        return $this->fetch();
    }
...
//            header("Location: " . U('Mobile/User/index'));
    ...
    public function add_address()
    {
        if (IS_POST) {
            ...
            if ($data['status'] != 1){
                ...
                $data['url']=U('/Mobile/Cart/cart2', array('address_id' => $data['result'],'goods_id'=>$goods_id,'goods_num'=>$goods_num,'item_id'=>$item_id,'qrid'=>$qrid,'action'=>$action));
                $this->ajaxReturn($data);
    ...
    */
    ...
    public function withdrawals()
    {
        ...
        $withdrawalsCount = M('withdrawals')->where($withdrawals_where)->count();
        ...
        if (IS_POST) {
            ...
            // 验证码是否正确
            if (isset($data["mobile"]) || isset($data['mobile_code'])) {
                $logic = new UsersLogic();
                $reg_sms_enable = tpCache('sms.regis_sms_enable');
        ...
        $this->assign('withdrawalsCount', $withdrawalsCount);    //提现次数
...
//        $user['count_leader'] = $user['first_leader'] + $user['second_leader'] + $user['third_leader'];//当前用户总
        $user['count_leader'] = $user['first_leader'] + $user['second_leader'];//当前用户总
//        $list = M('users')->whereOR(['first_leader'=>$user_id])->whereOR(['second_leader'=>$user_id])->whereOR(['third_leader'=>$user_id])->select();
            $list = M('users')->whereOR(['first_leader'=>$user_id])->whereOR(['second_leader'=>$user_id])->select();
        $users = '';
        $_list = array();
     ...
     * 用户代理区域列表
     */
    public function agency_area_list()
    {
        $user_id = $this->user_id;
        $level = M('users')->where(['user_id'=>$user_id])->find();
        if ($level['level'] == 5 || $level['level'] == 6) {
            // 代理区域表里代理的所有ID
            $agency_area =  M('agency_area')->where(array('level'=>$level['level'],'user_id'=>$user_id))->select();
        }

        // 重组数组查询
        if ($level['level'] == 5) {
            $arr = array_column($agency_area,'area_id');
            // 代理区域所有季度的奖金
            $quarter_money = M('quarter_region')->where(array('user_id'=>$user_id,'type'=>2))->sum('jackpot');
            $title = "区代季度奖";
        }elseif ($level['level'] == 6) {
            $arr = array_column($agency_area,'city_id');
            // 代理城市所有季度的奖金
            $quarter_money = M('quarter_region')->where(array('user_id'=>$user_id,'type'=>1))->sum('jackpot');
            $title = "市代季度奖";
        }
        $quarter_money = $quarter_money ? $quarter_money : 0;

        // 代理的所有区域的信息
        $list =  M('region')->where(array('id'=>array('in',$arr)))->select();
        $user = M('users')->where(['user_id'=>$user_id])->find();
        $countList = count($list);
        $user_list = M('users')->whereOR(['first_leader'=>$user_id])->whereOR(['second_leader'=>$user_id])->whereOR(['third_leader'=>$user_id])->select();
        $user_arr = array_column($user_list, 'user_id');
        $purchase=M('order')->where('user_id','IN',$user_arr)->where('order_status','in',['2','4'])->where('shipping_status&pay_status',1)->group('user_id')->count();//已购人数
        $this->assign('purchase', $purchase);
        $this->assign('user', $user);
        $this->assign('list', $list);
        $this->assign('countList', $countList);  
        $this->assign('quarter_money', $quarter_money);
        $this->assign('title',$title);

        return $this->fetch();
    }
    // ajax获取市或区的相关列表
    function ajax_agency_area(){
        $data = input('id');
        $where = array('user_id'=>$this->user_id,'region_id'=>$data);
        $quarter_region = M('quarter_region')->where($where)->select();
        $this->assign('quarter_region', $quarter_region);
        return $this->fetch();
    }
    //所有粉丝信息
...
//            'third_leader'=>$user_id,
        ...
        if($type == 1){
            ...
            if(!preg_match("/[^\d-., ]/",$account)&&strlen($account)<11){
                $count_leader =  M('users')->where(function ($query) {
                    $account = input('account');
                    $query->where('user_id',$account);
                    })->where(function ($query) {
                        $user_id = $this->user_id;
                        $query->whereOR('first_leader',$user_id)->whereOR('second_leader',$user_id);
                    })->select();
    ...
    {
        $parent_id = I('id/d');
        $parent_info = M('users')->where(array('user_id'=>$parent_id))->find();

        //是否已经绑定了上下级关系
        $user_info = M('users')->where('user_id',$this->user_id)->find();

        if(empty($parent_info)||empty($user_info))
...
//            $this->error('您已经存在上级，不可以继续绑定',U('index/index'));
        if($parent_id==$user_info['user_id'])//不能绑定自己
            $this->redirect(U('index/index'));

        $verification_parent = $this->verification_parent($this->user_id,$parent_id);
        if (!$verification_parent) 
            // 数据错乱,某个上级是自己
            $this->redirect(U('index/index'));

        $first_leader = M('users')->where(array('user_id'=>$parent_id))->find();
        ...
        $user_id=$this->user_id;
        M('users')->where('user_id',$user_id)->update($map);
        $this->redirect(U('index/index'));

        die;

        $this->success('您已绑定成为'.$parent_info['nickname'].'的下级',U('user/index'));
     ...
     * @return boolean
     */
    public function verification_parent($user_id,$parent_id)
    {
        $parent_info = M('users')->where(array('user_id'=>$parent_id))->find();

        if ($parent_info) {
            if (($parent_info['first_leader'] == $user_id) || ($parent_info['second_leader'] == $user_id) || ($parent_info['third_leader'] == $user_id)) {
                return false;
            }else{
                return $this->verification_parent($user_id,$parent_info['first_leader']);
            }
        }else{
            return ture;
        }
