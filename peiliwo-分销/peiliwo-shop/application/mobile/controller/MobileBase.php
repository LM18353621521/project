...
//        Session::start();
        ...
        $wx_qr = M('wx_user')->cache(true)->find(); //获取微信配置
        $this->assign('wx_qr',$wx_qr['qr']);
        $this->assign('headerpic',$wx_qr['headerpic']);
...
//            if(empty($user)){
//                if(!in_array(CONTROLLER_NAME,array('user','Article'))&&!in_array(ACTION_NAME,array('login','do_login','logout','reg','forget_pwd','find_pwd','set_pwd','verify'))){
//                    $this->error('您还没有登录，请先登录，建议升级',U('Mobile/User/login'));
//                }
//            }else{
//                if($user['level']<=1){
//                    if(CONTROLLER_NAME!='user'&&!in_array(ACTION_NAME,array('logout'))){
//                        $this->error('您目前等级不允许访问网站，建议升级',U('Mobile/User/logout'));
//                    }
//                }
//            }
//        }
...
//        $urlObj["scope"] = "snsapi_base";
    ...
    public function is_subscribe(){
        ...
        if($subscribe_info['subscribe']==0){
            $wx_follow_switch= tpCache("wechat_type.wx_follow_switch");
            $is_show=$wx_follow_switch;
        }else{
            $is_show=0;
        }
        return $this->ajaxReturn($is_show);
    }
