...
class Test extends Controller {
    ...
    public function testa(){
        echo "test";
        $user = M('users')->where(array('user_id'=>2616))->find();
//        dump($user);
        $oauth = M('oauth_users')->select();
        $oauth=M('oauth_users')->where(array('user_id'=>array('in',array(2662,2663,2665))))->select();
//        dump($oauth);
        echo "<br>";
        $user=M('users')->where(array('user_id'=>array('in',array(2662,2663))))->select();
        $user=M('users')->where(array('openid'=>'oSRIL0iGb34h5VHYOxdT365b3ODY'))->select();
//        dump($user);

        $list = M('agency_area')->alias('a')
            ->join('users u','a.user_id=u.user_id','left')
            ->field('a.id,a.city_id,a.area_id,u.user_id')
            ->select();
//        dump($list);
        $list = M('agency_area')->where(array('id'=>array('in',[4,21,22])))->delete();

    }
    public function kdniao_test(){
        include_once  "plugins/kdniao/kdniao.php"; // D:\wamp\www\svn_tpshop\www\plugins\payment\alipay\alipayPayment.class.php
        $kdniao= new \Kdniao();
        //884290204239
        //圆通：802012616404959726
        $requestData= "{'OrderCode':'','ShipperCode':'YTO','LogisticCode':'802012616404959726'}";
        dump($kdniao->getOrderTracesByJson($requestData));
    }
