...
class Order extends Base
{
    ...
    public function ajaxindex()
    {
        ...
        switch ($keyType) {
            case 'consignee':
                $condition['consignee'] = ['like', '%' . trim($keywords) . '%'];
                break;
            case 'consignee_tel':
                $condition['mobile'] = ['like', '%' . trim($keywords) . '%'];
                break;
            case 'order_sn':
                $condition['order_sn'] = ['like', '%' . trim($order_sn) . '%'];
                break;
            case 'order_prom_id':
                $condition['order_prom_id'] = trim($order_prom_id);
                break;
        }
    ...
    */
        ...
        if ($data['pay_status'] == 3) {
            ...
            if ($data['refund_type'] == 0) {
                ...
                if ($order['pay_code'] == 'weixin' || $order['pay_code'] == 'alipay' || $order['pay_code'] == 'alipayMobile') {
                    ...
                    $data = array('transaction_id' => $order['transaction_id'], 'total_fee' => $order['order_amount'], 'refund_fee' => $order['order_amount'], 'order_sn' => $order['order_sn']);
    ...
    public function detail($order_id)
    {
        ...
        if ($order['order_type'] == 1) {
            $user_level = M('user_level')->where(array('identity_id' => $identity_id))->find();
            if ($user_level['level_id'] == 5) {
                $agency_area = M('region')->alias('a')
                    ->join('region r1', 'a.parent_id=r1.id')
                    ->join('region r2', 'r1.parent_id=r2.id')
                    ->where(array('a.id' => $order['agency_area']))
                    ->field('a.name as district,r1.name as city,r2.name as province')
                    ->find();
            }
            if ($user_level['level_id'] == 6) {
                $agency_area = M('region')->alias('a')
                    ->join('region r1', 'a.parent_id=r1.id')
                    ->where(array('a.id' => $order['agency_area']))
                    ->field('a.name as city,r1.name as province')
                    ->find();
            }
            $this->assign('user_level', $user_level);
            $this->assign('agency_area', $agency_area);
        }

        if ($order['qrid'] > 0) {
            $recommend_user = M('users')->where(array('user_id' => $order['recommend_user_id']))->find();
    ...
    public function export_order()
    {
        ...
        if ($consignee_tel) {
            $where['mobile'] = ['like', '%' . $consignee_tel . '%'];;
        }
        ...
        switch ($keyType) {
            case 'consignee':
                $where['consignee'] = ['like', '%' . trim($keywords) . '%'];
                break;
            case 'consignee_tel':
                $where['mobile'] = ['like', '%' . trim($keywords) . '%'];
                break;
            case 'order_sn':
        ...
        I('order_status') != '' ? $where['order_status'] = I('order_status') : false;//订单状态
        I('pay_status') != '' ? $where['pay_status'] = I('pay_status') : false;//支付状态
        I('pay_code') != '' ? $where['pay_code'] = I('pay_code') : false;//支付方式
        I('shipping_status') != '' ? $where['shipping_status'] = I('shipping_status') : false;//发货状态
