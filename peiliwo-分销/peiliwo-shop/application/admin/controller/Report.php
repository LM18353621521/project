...
class Report extends Base{
	...
	public function cityTurnover(){
		...
		if ($quarter!="") {
			...
			// 季度值 默认最大值
			$quarter = M('quarter_region')->max('quarter');
			$where['quarter'] = $quarter;
		}
		$where['type'] = 1;

		$count = M('quarter_region')->where($where)->count();
		$page = new Page($count);
		$lists  = M('quarter_region')
				->where($where)
				->limit($page->firstRow.','.$page->listRows)
				->select();
		foreach ($lists as $key => $value) {
			$users = M('users')->where('user_id',$value['user_id'])->find();
			if (!empty($users)) {
				$lists[$key]['nickname'] = $users['nickname'];
			}else{
				$lists[$key]['nickname'] = '无';
			}
			$city = M('region')->where('id',$value['region_id'])->find();
			$province = M('region')->where('id',$city['parent_id'])->find();

			$lists[$key]['provincename'] = $province['name'];
			$lists[$key]['cityname'] = $city['name'];
		}
		$groups = M('quarter_region')
			->group('quarter')->select(); // 所有季度
		$quarterArr = array_column($groups,'quarter');
		...
		$this->assign('list',$lists);
		$this->assign('quarterArr',$quarterArr); //可选择的所有季度
		$this->assign('quarter',$quarter); // 当前查询的季度
     ...
     * @return mixed
     */
	public function districtTurnover(){
		$start_time = strtotime(I('start_time'));
        $end_time = strtotime(I('end_time'));
        $quarter = I('quarter','');
		...
		foreach ($lists as $key => $value) {
			...
			$district = M('region')->where('id',$value['region_id'])->find();
			$city = M('region')->where('id',$district['parent_id'])->find();
			$province = M('region')->where('id',$city['parent_id'])->find();

			$lists[$key]['provincename'] = $province['name'];
			$lists[$key]['cityname'] = $city['name'];
			$lists[$key]['districtname'] = $district['name'];
		}
		$groups = M('quarter_region')->group('quarter')->select(); // 所有季度
		$quarterArr = array_column($groups,'quarter');
...
//        echo $can_distribution;
		...
		$jackpot['sales_volume'] = $sales_volume_total;

		$this->assign('jackpot',$jackpot);
		$this->assign('inc_type','jackpot');
		return $this->fetch();
	}
	 ...
	 * @return mixed
	 */
	public function city_now_count(){
		//分奖时机
		$can_distribution = tpCache('distribut.can_distribution');
//        echo $can_distribution;
		...
		$count = M('order')->alias('a')
			->join('region r1','a.province=r1.id')
			->join('region r2','a.city=r2.id')
			->join('region r3','a.district=r3.id')
			->join('order_goods og','a.order_id=og.order_id')
			->where($where)
			->group('a.city')->count();
		$page = new Page($count);
		...
		$city_sale_count = M('order')->alias('a')
			...
			->join('region r3','a.district=r3.id')
			->join('order_goods og','a.order_id=og.order_id')
			->join('agency_area ag','a.city=ag.city_id','left')
			->join('users u','ag.user_id=u.user_id','left')
			->field('a.order_id,a.province,r1.name as province_name,a.city,r2.name as city_name,a.district,r3.name as district_name,og.rec_id,sum(og.sales_volume) as sales_volume,u.nickname')
			->where($where)
			->group('a.city')
			->limit($page->firstRow.','.$page->listRows)->select();
		...
		$this->assign('page',$page->show());
		$this->assign('total_count',$count);
		$this->assign('list',$city_sale_count);
		$this->assign('inc_type','city_now_count');
		return $this->fetch();
	 ...
	 * @return mixed
	 */
	public function district_now_count(){
		//分奖时机
		$can_distribution = tpCache('distribut.can_distribution');
//        echo $can_distribution;
		...
		$city_sale_count = M('order')->alias('a')
			...
			->join('region r3','a.district=r3.id')
			->join('order_goods og','a.order_id=og.order_id')
			->join('agency_area ag','a.district=ag.area_id','left')
			->join('users u','ag.user_id=u.user_id','left')
			->field('a.order_id,a.province,r1.name as province_name,a.city,r2.name as city_name,a.district,r3.name as district_name,og.rec_id,sum(og.sales_volume) as sales_volume,u.nickname')
			->where($where)
			->group('a.city')
		...
		$this->assign('total_count',$count);
		$this->assign('list',$city_sale_count);
		$this->assign('inc_type','district_now_count');
		return $this->fetch();
	...
	public function quarter_check(){
		$type = I('post.type');
		if($type=='count'){
			quarter_count();
			$this->success("操作成功!",U('jackpot'),3);
		}
		return $this->fetch();
	}
