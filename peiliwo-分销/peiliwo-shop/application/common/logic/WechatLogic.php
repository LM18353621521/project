...
class WechatLogic
{
    ...
    private function handleSubscribeEvent($msg)
    {
        ...
        if (!($user = M('users')->where('openid', $openid)->find())) {
            ...
            //不存在则创建个第三方账号
            M('OauthUsers')->save(array('oauth'=>$user['oauth'] , 'openid'=>$openid ,'user_id'=>$ret , 'unionid'=>$user['unionid'], 'oauth_child'=>'mp'));
        }
        setcookie('subscribe',null);
        session('subscribe', 1);// 当前这个用户是否关注了微信公众号
