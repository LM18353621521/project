...
class OrderLogic
{
	...
	public function addOrder($user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price=[],$user_note='',$pay_name='',$agency_area,$qrid=0)
	{
		...
		if($qrid>0){
			$user_give_vip = M('user_give_vip')->where(array('id'=>$qrid))->find();
			$recommend_user_id=$user_give_vip['user_id'];
		}
		...
		$data = array(
                ...
                'can_distribution' => $can_distribution, //升级判断
				'pay_name'         =>$pay_name,//支付方式，可能是余额支付或积分兑换，后面其他支付方式会替换

				'agency_area'      =>$agency_area,//代理地区id 市OR区
				'qrid'=>$qrid,//是否赠送名额下单
				'recommend_user_id'=>$recommend_user_id,//推荐人id
		);
		...
		//改变赠送名额的状态
		if($qrid>0){
			$res_give = M('user_give_vip')->where(array('id'=>$qrid))->update(array('use_time'=>time(),'status'=>2));
		}
		...
		foreach($cartList as $key => $val)
		{
            ...
            $data2['identity_id']        = $goods['identity_id']; // 身份产品id
            $data2['sales_volume']       = $goods['identity_id']>0?$goods['sales_volume']:$val['member_goods_price'];//销售业绩
			$order_goods_id              = M("OrderGoods")->insertGetId($data2);

			if($goods['identity_id']>0){
				$order_type=1;
			}
		}
		...
		//更新订单的类型
		if($order_type==1){
			$res_order = M('order')->where(['order_id'=>$order['order_id']])->update(array('order_type'=>$order_type));
		}
		...
		//分销开关全局
		try{
			$distribut_switch = distributCache('distribut.switch');
			$is_identity = M('order_goods')->where(array('order_id'=>$order['order_id'],'identity_id'=>0))->count();
			if($distribut_switch  == 1 && file_exists(APP_PATH.'common/logic/DistributLogic.php'))
			{
				...
				if($order['qrid']==0){
					if(!empty($is_identity)){//判断商品中是否有身份产品
						$distributLogic->general_prize($order); // 分成分佣记录
					}else{
						$distributLogic->Referral_bonus($order);//推荐分佣记录
					}
				}else{
					$distributLogic->give_vip_prize($order);//赠送名额分佣记录
				}
			}
		}catch(\Exception $e)
		{
