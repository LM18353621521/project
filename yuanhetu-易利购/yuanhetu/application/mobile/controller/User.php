...
//        dump($user['result']);exit;
        ...
        $mobile = Db::name('users')->where(['user_id' => $user['result']['first_leader']])->value('account');
    ...
    //收益积分明细 
    //1：充值；2：申请提现；3：提现失败返还 4：直推下级业绩返点 5：代理商补贴  6 : 直推代理商收入 7：月销售额收入；8：收益积分购买产品；9：转出；10：转入； 11：寄售商品收入 12：合伙人补贴；13：转让手续费 14 : 回收出售商品；15消费专区收益积分购买；16：代理商团队奖；17：董事团队奖；18：直推奖；19：团队新增业绩奖励；20：平台分红,21:余额兑换
        ...
        foreach ($logs as $k=>$v) {
            ...
            switch ($v['type']) {
                ...
                case 14 :
                    ...
                    break;
                case 15 :
                    $logs[$k]['type_str'] = "消费专区购买";
                    break;
                case 16 :
                    $logs[$k]['type_str'] = "代理商团队奖";
                    break;
                case 17 :
                    $logs[$k]['type_str'] = "董事团队奖";
                    break;
                case 18 :
                    $logs[$k]['type_str'] = "直推奖（奖项一）";
                    break;
                case 19 :
                    $logs[$k]['type_str'] = "团队新增业绩奖励（奖项三）";
                    break;
                case 20 :
                    $logs[$k]['type_str']= "平台分红(奖项二)";
                    break;
                case 21 :
                    $logs[$k]['type_str']= "余额兑换";
                    break;
                case 22 :
                    $logs[$k]['type_str']= "平台分红(奖项四)";
                    break;
                default:
                    $logs[$k]['type_str'] = "收入";
                    break;
    ...
    public function reg()
    {
        ...
        if (IS_POST) {
            ...
            if(!preg_match('/[0-9A-Za-z]$/',$account)){
                $this->ajaxReturn(['msg'=>'账号只能包含字母、数字']);
            }
            if(strlen($account)<6||strlen($account)>20){
                $this->ajaxReturn(['msg'=>'账号长度为6到20位']);
            }
            if(!check_mobile($mobile)){
                $this->ajaxReturn(['msg'=>'手机号码不正确']);
            }
            ...
            $where_account['user_id|account'] =$account;
            $count = Db::name("users")->where('account',$account)->whereOr('user_id',$account)->count();
...
//                $this->ajaxReturn(['msg'=>'推荐人不能为空！']);
            ...
            //账户
            if (empty($account)) {
                $this->ajaxReturn(['msg'=>'账号不能为空！']);
            } else {
                //是否存在
                $account_occupy = M("users")->field("user_id")->where(array("account"=>$account))->find();
                if ($account_occupy) $this->ajaxReturn(['msg'=>'账号已存在！']);
            }
    ...
    public function add_address()
    {
        if (IS_POST) {
            ...
            if ($data['status'] != 1){
                ...
                $data['url']=U('/Mobile/Cart/cart2', array('address_id' => $data['result'],'goods_id'=>$goods_id,'goods_num'=>$goods_num,'item_id'=>$item_id,'action'=>$action,'setmeal_id'=>$setmeal_id));
                $this->ajaxReturn($data);
                ...
                $this->ajaxReturn($data);
            } elseif($_POST['source'] == 'mention_goods'){
                $data['url']= U('/Mobile/Member/mention_goods', array('address_id' => $data['result'],'id'=>$goods_id,'consignment_id'=>$consignment_id));
                $this->ajaxReturn($data);
            }else{
    ...
    public function userinfo()
    {
        ...
        if (IS_POST) {
            ...
            if (!empty($email)) {
                ...
                $c && exit(json_encode(['code'=>-1,'msg'=>'邮箱已被使用']));
                //$this->error("邮箱已被使用");
            }
            if (!empty($mobile)) {
...
//                    $this->error($check_code['msg']);
    ...
    */
    ...
    //购物积分明细
    //1：收入；2：提现 ；4：平台管理费；5：提现手续费；8：购买商品 9：寄售返余额,10-兑换佣金
        ...
        foreach ($logs as $k=>$v) {
            ...
            switch ($v['type']) {
                ...
                case 3 :
                    ...
                    break;
                case 4 :
                    $logs[$k]['type_str'] = "平台管理费";
                    break;
                case 5 :
                    $logs[$k]['type_str'] = "提现手续费";
                    break;
                case 8 :
                    $logs[$k]['type_str'] = "购买商品";
                    break;
                case 9 :
                    $logs[$k]['type_str'] = "寄售返收益";
                    break;
                case 10 :
                    $logs[$k]['type_str'] = "兑换余额";
                    break;
                default:
                    $logs[$k]['type_str'] = "收入";
                    break;
    ...
    public function password()
    {
        if (IS_POST) {
            ...
            if($user['password']!=encrypt($old_password,$user['oauth'])){
                exit(['code' => -2, 'msg' => '原密码验证错误！']);
            }
            if($user['oauth']){
                M('users')->where("user_id", $user['user_id'])->save(array('password' => encrypt($password),'oauth'=>''));
            }else{
    ...
    public function password2()
    {
        if (IS_POST) {
            ...
            $user = M('users')->where("user_id", $this->user_id)->find();
            if($user['paypwd']!=encrypt($old_password,$user['oauth'])){
                exit(['code' => -2, 'msg' => '原密码验证错误！']);
            }   
            if($user['oauth']){
                M('users')->where("user_id", $user['user_id'])->save(array('paypwd' => encrypt($password),'oauth'=>''));
            }else{
    ...
    function forget_pwd()
    {
        ...
        if (IS_POST) {
            if (!empty($username)) {
                ...
                $user = M('users')->where("email", $username)->whereOr('account', $username)->find();
                if ($user) {
    ...
    public function withdrawals()
    {
        ...
        $reserve_funds=$this->user['distribut_money']-$ylg_spstem_role['reserve_funds'];
        ...
        if (IS_POST) {
            ...
            $money=Db::name('withdrawals')->whereTime('create_time', 'today')->where('user_id',$this->user_id)->sum('money');
            /*if(($money+$data['money'])>$ylg_spstem_role['withdrawal_limit']){
                $this->ajaxReturn(['status'=>0,'msg'=>'每天最多可提现'.$ylg_spstem_role['withdrawal_limit']]);
            }*/

            $data['taxfee'] = $data['money']*$taxfee; //手续费

            if(encrypt($data['paypwd'],$this->user['oauth']) != $this->user['paypwd']){
                $this->ajaxReturn(['status'=>0,'msg'=>'支付密码错误']);
            }

            /*if ($data['money'] > $this->user['distribut_money']-$ylg_spstem_role['reserve_funds']) {
                $this->ajaxReturn(['status'=>0,'msg'=>"你最多可提现{$reserve_funds}账户余额."]);
                exit;
            }*/
            if (($data['money']) > ($this->user['distribut_money']-$data['taxfee'])) {
            ...
            $num=$data['money']+$data['taxfee'];
            // 启动事务
            Db::startTrans();
            try{
                $res = Db::name('withdrawals')->insertGetId($data);
                Db::name('users')->where('user_id',$this->user_id)->setDec('distribut_money',$num);
                shoppinglog($res,$this->user_id, -$data['money'], 2, $this->user['distribut_money'],$this->user['distribut_money']-$data['money']);
                //提现手续费
                shoppinglog($res,$this->user_id, -$data['taxfee'], 5, $this->user['distribut_money']-$data['money'],$this->user['distribut_money']-$num);
...
//        }
        ...
        $this->assign('distribut_money', $this->user['distribut_money']);    //用户余额
    ...
    {
        $commodity = 0;//tpCache('ylg_spstem_role.commodity');转让手续费
        if (IS_POST) {
            ...
            $user2 = M('users')->where("user_id", $data['user_name'])->whereOr("account", $data['user_name'])->find();
            if(empty($user2)){
            ...
            /*if(empty($user2['second_leader'])&&empty($this->user['second_leader'])){
                $this->ajaxReturn(['status'=>0,'msg'=>'只能转给团队的人']);exit;
            }*/

            /*if($this->user['second_leader']){
                $arr=explode(',',$this->user['second_leader']);
            }else{
                $arr=[];
            }

            if($user2['second_leader']){
                $arr2=explode(',',$user2['second_leader']);
            }else{
                $arr2=[];
            }

            if(in_array($user2['user_id'],$arr)||in_array($this->user_id,$arr2)){
    ...
    public function transfer_list()
    {
        $page = (int)I('p') ? (int)I('p') : 0;
        $list = 15;
        $where='(userId='.$this->user_id.' or toUserId='.$this->user_id.') and type=1';
        $list = M('transfer')->where($where)->order("id desc")->limit($page*$list, $list)->select();
        foreach ($list as $key => $value) {
     ...
     * 积分互转
     */
    public function transfer_integral()
    {
        $commodity = 0;//tpCache('ylg_spstem_role.commodity');转让手续费
        if (IS_POST) {
            if (!empty(session("transfer")) && time() - session("transfer") <= 5) {
            ...
            session("transfer", time());
            $reserve_funds=$this->user['frozen_money'];
            $data = I('post.');      
            if(empty($data['money'])||$data['money']<=0){
                $this->ajaxReturn(['status'=>0,'msg'=>'转让金额要大于0']);exit;
            }     
            ...
            if ($data['money'] > $this->user['frozen_money']) {
                $this->ajaxReturn(['status'=>0,'msg'=>"你最多可转让{$reserve_funds}"]);exit;
            }
            
            if($reserve_funds<=0){
                $this->ajaxReturn(['status'=>0,'msg'=>'积分不足']);exit;
            }
            $user2 = M('users')->where("user_id", $data['user_name'])->whereOr("account", $data['user_name'])->find();
            if(empty($user2)){
                $this->ajaxReturn(['status'=>0,'msg'=>'收款用户不存在']);exit;
            ...
            /*if (($data['money']*(1+ $commodity) )> $this->user['frozen_money']) {
                $this->ajaxReturn(['status'=>0,'msg'=>"积分不足,无法支付手续费"]);
                exit;
            }*/
            $num=$data['money'];
            ...
            try{
                ...
                $res=M("transfer")->add($data2);//添加转让
                //转出
                Db::name('users')->where('user_id',$this->user_id)->setDec('frozen_money',$num);
                integrallog($res,$this->user_id, -$num, 11, $this->user['frozen_money'],$this->user['frozen_money']-$num);

                if($num2>0){
                    //手续费
                    Db::name('users')->where('user_id',$this->user_id)->setDec('frozen_money',$num2);
                    integrallog($res,$this->user_id, -$num2, 13, $this->user['frozen_money']-$num,$this->user['frozen_money']-$num-$num2);
                }

                //转入
                Db::name('users')->where('user_id',$user2['user_id'])->setInc('frozen_money',$num);
                integrallog($res,$user2['user_id'], $num, 12, $user2['frozen_money'],$user2['frozen_money']+$num);

                // 提交事务
                Db::commit();  
                $this->ajaxReturn(['status'=>1,'msg'=>"转让成功",'url'=>U('User/transfer_integral')]);
                exit;  
            } catch (\Exception $e) {
                // 回滚事务
                Db::rollback();
        ...
        $this->assign('frozen_money', $this->user['frozen_money']);    //用户余额
        $this->assign('handling_fee', $commodity*100);    //手续费
        return $this->fetch();
    }
    /**
     * 积分互转记录
     */
    public function transfer_integral_list()
    {
        $page = (int)I('p') ? (int)I('p') : 0;
        $list = 15;
        $withdrawals_where['userId'] = $this->user_id;
        $withdrawals_where['type'] =2;
...
//    提货
    ...
    public function level_apply()
    {
        ...
        //申请等级
        $level = M('user_level')->where(array('level_id'=>3))->find();
        //缴纳金额
        $pay_money = $level['discount'];
        ...
        $has_apply = M('user_level_apply')->where($where)->find();

        if(IS_POST){
            $data =   I('post.');
            $limit_num = tpCache('ylg_spstem_role.limit_num');
            $count = M('users')->where(array('level'=>3))->count();
            if($count>$limit_num){
                $this->ajaxReturn(['status'=>0,'msg'=>"高级会员人数已满"]);
            }

            if($has_apply&&$has_apply['status']==0){
                $this->ajaxReturn(['status'=>0,'msg'=>"您的申请正在审核中，请勿重复申请"]);
            }

            /*$file =request()->file('img');
            if($file){
                $info = $file->move(ROOT_PATH . 'public' . DS . 'upload/img');
                $data['pay_pic'] ='public' . DS . 'upload/img\\'.$info->getSaveName();
            }else{
                $this->ajaxReturn(['status'=>0,'msg'=>"未上传收款凭证！"]);
            }*/
            if($user['user_money']<$pay_money){
                $this->ajaxReturn(['status'=>0,'msg'=>"余额不足，无法升级"]);
            }
            $data['user_id']=$this->user_id;
            ...
            $res=M('user_level_apply')->add($data);
            $res2=M('users')->where(array('user_id'=>$this->user_id))->setDec('user_money',$pay_money);
            if($res&&$res2){
                balancelog($res, $this->user_id, -$pay_money, 24, $user['user_money'],$user['user_money']-$pay_money);
        ...
        if($has_apply&&$has_apply['status']==0){
            $this->error('您的申请正在审核中，请勿重复申请',U('User/index'));
        }

        if($user['level']>=$level['level_id']){
            $this->error('您的等级已达到或超过'.$level['level_name'].',无需申请升级',U('User/index'));
        }
    ...
    {
        //转换比例
        $exchange_rate = tpCache('ylg_spstem_role.goods_integral');
        $user = M('users')->where(array('user_id'=>$this->user_id))->find();
        if(IS_POST){
            $data =   I('post.');
            $num =$data['num'];
            ...
            if($user['distribut_money']<$num){
                $this->ajaxReturn(['status'=>0,'msg'=>"可用额度不足"]);
            }

            $after_num = $num*$exchange_rate;

            $data['user_id']=$this->user_id;
            $data['num']=$num;
            $data['exchange_rate']=$exchange_rate;
            $data['after_num']=$after_num;
            $data['create_time']=time();
            $data['status']=1;

            DB::startTrans();
            $res=M('asset_exchange')->add($data);
            $user_uppdate = array(
                'distribut_money'=>array('exp','distribut_money-'.$num),
                'user_money'=>array('exp','user_money+'.$after_num),
            );
            $res1 = M('users')->where(array('user_id'=>$this->user_id))->update($user_uppdate);
            $res2=shoppinglog($res, $this->user_id, -$num, 10, $user['distribut_money'],$user['distribut_money']-$num);
            $res3=balancelog($res, $this->user_id, $after_num, 21, $user['user_money'],$user['user_money']+$after_num);
            ...
            if($res&&$res1&&$res2&&$res3){
                ...
                DB::rollback();
                $this->ajaxReturn(['status'=>0,'msg'=>"兑换失败"]);
            }
        }
     ...
     * 授信使用记录
     */
    public function asset_exchange_list()
    {
        $user_id = $this->user_id;
        if (IS_POST) {
            $page = (int)I('p') ? (int)I('p') : 0;
            ...
            $list = M('asset_exchange')->where($where)->whereOr($where)->order("id desc")->limit($page*$list, $list)->select();
            foreach ($list as $key => $value) {
                $list[$key]['createTime'] = date('Y-m-d H:i',$value['create_time']);
            }
            $this->ajaxReturn(['data'=>$list,'user_id'=>$this->user_id]);
        }
        return $this->fetch();
