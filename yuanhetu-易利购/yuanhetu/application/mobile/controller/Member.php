    ...
    */
    ...
    public function sell_goods()
    {
        ...
        if ($this->request->isAjax()) {
            $p = $this->request->post('p');
            $num = 15;
            $goodslist = Db::name('goods')->where("type_id = 6 AND status = 0 AND is_on_sale = 1")->where('agentend_time','> time',date('Y-m-d H:i:s'))->field('goods_id,goods_name')->limit($num * $p, $num * $p + $num)->select();
            foreach ($goodslist as $k => $v) {
                $goodslist[$k]['setmeal'] = Db::name('goods_setmeal')->where('goods_id', $v['goods_id'])->select();
            }
            foreach ($goodslist as $key => $val) {
...
//                   ->where("setmeal_id = {$goodslist[$i]['id']} AND user_id = {$this->user_id} AND type = 0")->select();
                ...
                if($consignment){
                    if($data['num'] > $consignment['surplus_num']){
                        $this->error('可提货数量不足!');
                    }
                }
                //提货
