...
class Cart extends MobileBase {
    ...
    public function cart2(){
        ...
        if(empty($address)){
            header("Location: ".U('Mobile/User/add_address',array('source'=>'cart2','action'=>$action,'goods_id'=>$goods_id,'goods_num'=>$goods_num,'item_id'=>$item_id,'setmeal_id'=>$setmeal_id)));
            exit;
        }else{
        ...
        if($action == 'buy_now'){
            ...
            if($goods['type_id'] == 6 && $goods['status'] == 1){
                ...
                $order_num = Db::name('order')->alias('o')->join('order_goods g','o.order_id = g.order_id')
                    ->where("user_id = {$this->user_id} AND goods_id = {$goods['goods_id']} and type=1 and pay_status=1")->field('sum(goods_num) sums')->find();
...
//        dump($result);exit;
        ...
        $car_price = array(
            ...
            'net_income'     => $result['result']['net_income'],// 商品总净利
            'goods_integral'     => $result['result']['goods_integral'],// 商品购物积分
    ...
    public function cart4(){
        ...
        if(empty($order_id)){
            ...
            $setmeal = Db::name('goods_setmeal')->alias('s')->join('goods g','g.goods_id = s.goods_id')->where('id',$list['setmeal'])->find();
             $order_num = Db::name('order')->alias('o')->join('order_goods g','o.order_id = g.order_id')
                ->where("user_id = {$this->user_id} AND g.goods_id ={$setmeal['goods_id']} AND g.setmeal_id = {$list['setmeal']}")->field('sum(goods_num) sums')->find();
        ...
        $goods=M('goods')->alias('g')->join('tp_order_goods o','o.goods_id=g.goods_id')->where('o.order_id',$order_id)->field('g.*,o.goods_num')->find();
        $order_num = Db::name('order')->alias('o')->join('order_goods g','o.order_id = g.order_id')
            ->where("user_id = {$this->user_id} AND goods_id = {$goods['goods_id']} and type=1 and pay_status=1")->field('sum(goods_num) sums')->find();
        if($goods['goods_num']+$order_num['sums'] > $goods['limit_num'] || $goods['goods_num'] > $goods['limit_num']){
            $this->error('每人限购'.$goods['limit_num'].'件!');
        }
