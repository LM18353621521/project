...
function user_upgrade($id)
{
    ...
    if($user['level']==1){
        ...
        if($user['pay_points']>=$user_level2['amount']){
            ...
            return vpay_level_log($user['user_id'],$user['account'],'前台升级',$user['level'],$user['level']+1,2);
        }else{
            return 0;
        }
    }elseif($user['level']==3){
        ...
        //所有直推团队
        $list=Db::name('users')->where(['first_leader'=>['in',$user['user_id']]])->order('performance desc')->select();
        $i=0;
        foreach ($list as $key => $value) {
            //直推团队业绩
            if(($value['performance']+$value['monthly_performance'])>=$user_level2['is_promote']){
                $i++;
                //满足条件的直推团队
                if($i>=$user_level2['region_code']){
                    Db::name('users')->where(['user_id'=>$user['user_id']])->setInc('level');
                    return vpay_level_log($user['user_id'],$user['account'],'前台升级',$user['level'],$user['level']+1,2);
                }
            }else{
    ...
    if($num < 0&&in_array($type,$arr)){
        ...
        if($type==15){
            ...
            if($user['first_leader']>0){
                //直推奖 奖项一
                $user2=M('users')->where(array("user_id"=>$user['first_leader']))->find();
                $money=$system['consumer2']*abs($num);
                M('users')->where(array("user_id"=>$user2['user_id']))->inc('user_money',$money)->update();
                balancelog($user2['user_id'],$user2['user_id'],$money,18,$user2['user_money'],$user2['user_money']+$money);
            }
            if($user['second_leader']){
                ...
                foreach ($second_leader as $key => $value) {
                    //找到所有上级
                    $pid=Db::name('users')->where(['user_id'=>['in',$value]])->find();
                    $total_mance = $pid['performance']+$pid['monthly_performance'];
                    if($total_mance >= $system['agent_num2']){
                        if($state){
                            $money2=$system['agent_partner2']*abs($num);
                            if($money2>0){
                                //奖项三 无限层2
                                M('users')->where(array("user_id"=>$pid['user_id']))->inc('user_money',$money2)->update();
                                balancelog($pid['user_id'],$pid['user_id'],$money2,19,$pid['user_money'],$pid['user_money']+$money2);
                            }
                            break;
                        }else{
                            $money2=($system['agent_partner1']+$system['agent_partner2'])*abs($num);
                            if($money2>0){
                                //奖项三 无限层2
                                M('users')->where(array("user_id"=>$pid['user_id']))->inc('user_money',$money2)->update();
                                balancelog($pid['user_id'],$pid['user_id'],$money2,19,$pid['user_money'],$pid['user_money']+$money2);
                    ...
                    if($total_mance >= $system['agent_num1'] && $state == 0){
                        $money2=$system['agent_partner1']*abs($num);
                        if($money2>0){
                            //奖项三 无限层1
                            M('users')->where(array("user_id"=>$pid['user_id']))->inc('user_money',$money2)->update();
                            balancelog($pid['user_id'],$pid['user_id'],$money2,19,$pid['user_money'],$pid['user_money']+$money2);
            ...
            //累积总业绩（抢购区和批发区业绩）
            M('users')->where(array("user_id"=>$userId))->inc('monthly_performance',abs($num))->update();
        }
        //累计业绩给上级（抢购区和批发区）
        if($user['second_leader']){
            M('users')->where("user_id in ({$user['second_leader']}) and user_id<>$userId")->inc('performance',abs($num))->update();
            //获取团队上级
            $list=explode(',',$user['second_leader']);
            krsort($list);
...
function calculate_price($user_id = 0, $order_goods, $shipping_code = '', $shipping_price = 0, $province = 0, $city = 0, $district = 0, $pay_points = 0, $user_money = 0, $coupon_id = 0)
{
    ...
    foreach ($order_goods as $key => $val) {
        ...
        $order_goods[$key]['net_income'] = $val['goods_num'] * $val['market_price'];    // 小计 总净利
        $order_goods[$key]['shop_integral'] = $val['goods_num'] * $val['goods']['cost_price'];    // 小计 购物积分
        ...
        $goods_integral += $order_goods[$key]['shop_integral']; // 商品总购物积分
        $quota += $order_goods[$key]['quota']; // 商品总配额
        $net_income += $order_goods[$key]['net_income']; // 商品总配额
    ...
    $result = array(
        ...
        'goods_integral' => $goods_integral, // 商品总购物积分
        'quota' => $quota, // 商品总配额
