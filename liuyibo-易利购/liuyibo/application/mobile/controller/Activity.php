...
class Activity extends MobileBase
{
    ...
    //专区转换 //每日触发
    public function zone_conversion()
    {
    ...
    //库存为0修复
    public function inventory_repair()
    {
        $id=I('id');
        if($id){
            $surplus_num=M('goods_consignment')->where('goods_id',$id)->sum('surplus_num');
            $store_count=M('goods')->where('goods_id',$id)->value('store_count');
            if($store_count===0||$store_count==='0'){
                $res=M('goods')->where('goods_id',$id)->update(['store_count'=>$surplus_num]);
                echo $res;
            }
            dump($surplus_num);dump($store_count);
        }
    }
}