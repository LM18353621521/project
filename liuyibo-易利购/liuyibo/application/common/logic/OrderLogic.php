...
class OrderLogic
{
	...
	public function cancel_order($user_id,$order_id,$action_note='您取消了订单'){
        ...
        if($order['type'] == 2) {
            ...
            if (tpCache('shopping.reduce') == 1 && $order['type'] == 2) {
	...
	public function addReturnGoods($rec_id,$order)
	{
		...
		if($data['type'] < 2){
            $data['refund_money'] = $order_goods['goods_num']*$order_goods['goods_price'];//收益积分
            $data['refund_integral'] = $order_goods['goods_num']*$order_goods['cost_price'];//兑换积分
			...
			if($order['order_amount']>0 && !empty($order['pay_code'])){
				$data['refund_money'] = $rate*$order['goods_price'];//退款金额
				$data['refund_deposit'] = $rate*$order['user_money'];//该退余额支付部分
				$data['refund_integral'] = floor($rate*$order['cost_price']);//该退积分支付
			}else{
	...
	public function addOrder($user_id,$suppliers_id,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price=[],$user_note='',$pay_name='',$order_type = 0,$start_server_time = 0,$end_server_time = 0,$type)
	{
		...
		$data = array(
				...
				'shop_integral'      =>$car_price['goods_integral'],//'商品总兑换积分',
