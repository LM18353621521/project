...
class UsersLogic extends Model
{
    ...
    public function login($username,$password){
        ...
        if(!$user){
            ...
            $result = array('status'=>-3,'msg'=>'账号已被禁用！');
        }else{
            //查询用户信息之后, 查询用户的登记昵称
            $levelId = $user['level'];
    ...
    public function add_address($user_id,$address_id=0,$data){
        ...
        {
            $order=M('self_mention_order')->where("user_id=$user_id and (mobile='' or mobile is null)")->column('self_mention_sn');
            if(!empty($order)){
                $self_mention_sn=implode(',',$order);
                $addr['province']=$post['province'];
                $addr['city']=$post['city'];
                $addr['district']=$post['district'];
                $addr['twon']=$post['twon'];
                $addr['address']=$post['address'];
                $addr['consignee']=$post['consignee'];
                $addr['mobile']=$post['mobile'];
                M('self_mention_order')->where("self_mention_sn in ($self_mention_sn)")->update($addr);
            }
