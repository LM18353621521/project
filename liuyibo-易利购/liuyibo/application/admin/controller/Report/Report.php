...
class Report extends Base{
    ...
    public function sales_log(){
        ...
        $sale_data['total_sales'] = M('order')->where(['order_status'=>['in',[2,4]]])->sum('total_amount');//历史
        $sale_data['today_amount'] = M('order')->where("add_time>$now AND order_status in(2,4)")->sum('total_amount');//今日销售总额
        $sale_data['today_order'] = M('order')->where("add_time>$now AND order_status in(2,4)")->count();//今日订单数
        $sale_data['cancel_order'] = M('order')->where("add_time>$now AND order_status=3")->count();//今日取消订单
        ...
        $res = Db::name("order".$select_year)
            ->field(" COUNT(*) as tnum,sum(total_amount) as amount, FROM_UNIXTIME(add_time,'%Y-%m-%d') as gap ")
            ->where(" add_time >$begin and add_time < $end AND order_status in(1,2,4) ")
            ->group('gap')
            ->select();
        $tnum=0;
        $tamount=0;
	...
	public function expense_log(){
        ...
        if($keyword){
            if($keytype){
                ...
                $map['mobile|mobile']=array('like','%'.$keyword.'%');
            }
        }
        ...
        $lists  = M('balancelog')->alias('a')
            ...
            ->field('a.id,sum(num) as total_num,u.user_id,u.mobile as account,u.mobile')
    ...
    public function expense_logs(){
        ...
        $lists  = M('balancelog')->alias('a')
            ->join('users u','a.userId=u.user_id','left')
            ->where($map)->limit($page->firstRow.','.$page->listRows)
            ->field('a.*,u.user_id,u.mobile as account,u.mobile')
            ->select();
