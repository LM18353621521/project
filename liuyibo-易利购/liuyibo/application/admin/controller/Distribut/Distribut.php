...
class Distribut extends Base
{
    ...
    public function tree()
    {
        ...
        foreach ($list as $key => $value) {
            $list[$key]['num1']=M('users')->where('first_leader',$value['user_id'])->count();
            $list[$key]['num2']=count(get_team($value['user_id'],2));
        }
        $this->assign('list', $list);
        return $this->fetch();
    }
    ...
    public function ajax_lower()
    {
        ...
        foreach ($_list as $key2 => $value2) {
            $_list[$key2]['num1']=M('users')->where('first_leader',$value2['user_id'])->count();
            $_list[$key2]['num2']=count(get_team($value2['user_id'],2));
        }
