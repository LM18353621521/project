...
class User extends Base {
    ...
    public function ajaxindex(){
        ...
        if(I('id1')&&I('id2')){
            $condition['user_id']=[['>=',I('id1')],['<=',I('id2')]];
        }
        if(I('start_time')&&I('end_time')){
            $condition['reg_time']=['between',[strtotime(I('start_time')),strtotime(I('end_time'))]];
        }
        if(!empty($condition)){
            session('user_where',$condition);
        }
        ...
        //  搜索条件下 分页赋值
        if((I('id1')&&I('id2'))||(I('start_time')&&I('end_time'))){
        ...
        foreach ($userList as $k => $v) {
            //获取团队（所有下线）
            $userids2=get_team($v['user_id'],2);
            $ids2=implode(',',$userids2);

            if(empty($ids2)){
                $userList[$k]['team_performance']=0;
            }else{
                $userList[$k]['team_performance']=Db::name('users')->where(['user_id'=>['in',$ids2]])->sum('pay_points');
            }
        }
...
//        dump($condition);die;
    ...
    /*
     * 收益积分明细
     * 1：充值；2：申请提现；3：提现失败返还 4：直推下级业绩返点 5：代理商补贴  6 : 直推代理商收入 7：月销售额收入；8：收益积分购买产品；9：转出；10：转入； 11：寄售商品收入 12：合伙人返点1；13：转让手续费 14 : 回收出售商品；15消费专区收益积分购买；16：代理商团队奖；17：董事团队奖：18：兑换专区退款；19：个人所得税；20：合伙人返点2；21：直推返点
     */
            ...
            foreach ($list as $k=>$v) {
                ...
                switch ($v['type']) {
                    ...
                    case 7 :
                        ...
                        break;
                    case 8 :
                        $list[$k]['type_str'] = "代理专区购买";
                        $list[$k]['operator'] = $v['reflectId'];
                        break;
                    case 9 :
                        $list[$k]['type_str'] = "转出".'('.$operator2.')';
                        break;
                    case 10 :
                        $list[$k]['type_str'] = "转入".'('.$operator2.')';
                        break;
                    case 11 :
                        $list[$k]['type_str'] = "寄售商品收入";
                        break;
                    case 12 :
                        $list[$k]['type_str'] = "合伙人返点1";
                        break;
                    case 13 :
                    ...
                    case 17 :
                        ...
                        break;
                    case 18 :
                        $list[$k]['type_str'] = "兑换专区退款";
                        break;
                    case 19 :
                        $list[$k]['type_str'] = "个人所得税";
                        break;
                    case 20 :
                        $list[$k]['type_str'] = "合伙人返点2";
                        break;
                    case 21 :
                        $list[$k]['type_str'] = "直推返点".'('.$v['reflectId'].')';
                        break;
                    case 22 :
                        $list[$k]['type_str'] = "兑换专区购买";
                        $list[$k]['operator'] = $v['reflectId'];
                        break;
                    default:
                        $list[$k]['type_str'] = "收入";
    ...
    /*
     * 兑换积分明细
     * 1：收入；2：支出 ；3；直推返兑换积分；8：购买商品 9：寄售返兑换积分；18：兑换专区退款；
     */
            ...
            foreach ($list as $k=>$v) {
                ...
                switch ($v['type']) {
                    ...
                    case 8 :
                        ...
                        break;
                    case 9 :
                        $list[$k]['type_str'] = "寄售返兑换积分";
                        $list[$k]['operator'] = $v['reflectId'];
                        break;
                    case 4 :
                        $list[$k]['type_str'] = "购物返兑换积分";
                        $list[$k]['operator'] = $v['reflectId'];
                        break;
                    default:
                        $list[$k]['type_str'] = "收入";
...
//        $show = $Page->show();
    ...
    public function export_user(){
        $where=session('user_where');
    	$strTable ='<table width="500" border="1">';
    	$strTable .= '<tr>';
    	$strTable .= '<td style="text-align:center;font-size:12px;width:120px;">ID</td>';
    	$strTable .= '<td style="text-align:center;font-size:12px;" width="100">会员账号</td>';
    	$strTable .= '<td style="text-align:center;font-size:12px;" width="*">会员等级</td>';
    	$strTable .= '<td style="text-align:center;font-size:12px;" width="*">收益积分</td>';
    	$strTable .= '<td style="text-align:center;font-size:12px;" width="*">配额</td>';
    	$strTable .= '<td style="text-align:center;font-size:12px;" width="*">兑换积分</td>';
    	$strTable .= '<td style="text-align:center;font-size:12px;" width="*">上月业绩</td>';
    	$strTable .= '<td style="text-align:center;font-size:12px;" width="*">当月业绩</td>';
    	$strTable .= '<td style="text-align:center;font-size:12px;" width="*">团队流水</td>';
    	$strTable .= '<td style="text-align:center;font-size:12px;" width="*">上级ID</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;" width="*">注册时间</td>';
    	$strTable .= '</tr>';
    	$count = M('users')->where($where)->count();
    	$p = ceil($count/5000);
        $level=['0'=>'','1'=>'消费者','2'=>'消费商','3'=>'代理商','4'=>'合伙人','5'=>'董事'];
    	for($i=0;$i<$p;$i++){
    		$start = $i*5000;
    		$end = ($i+1)*5000;
    		$userList = M('users')->where($where)->order('user_id desc')->limit($start.','.$end)->select();
    		if(is_array($userList)){
    			foreach($userList as $k=>$val){
                    $userids2=get_team($val['user_id'],2);
                    $ids2=implode(',',$userids2);

                    if(empty($ids2)){
                        $team_performance=0;
                    }else{
                        $team_performance=Db::name('users')->where(['user_id'=>['in',$ids2]])->sum('pay_points');
                    }

    				$strTable .= '<tr>';
    				$strTable .= '<td style="text-align:center;font-size:12px;">'.$val['user_id'].'</td>';
    				$strTable .= '<td style="text-align:left;font-size:12px;">'.$val['mobile'].' </td>';
    				$strTable .= '<td style="text-align:left;font-size:12px;">'.$level[$val['level']].'</td>';
    				$strTable .= '<td style="text-align:left;font-size:12px;">'.$val['user_money'].'</td>';
    				$strTable .= '<td style="text-align:left;font-size:12px;">'.$val['frozen_money'].'</td>';
    				$strTable .= '<td style="text-align:left;font-size:12px;">'.$val['distribut_money'].'</td>';
    				$strTable .= '<td style="text-align:left;font-size:12px;">'.$val['sales'].'</td>';
    				$strTable .= '<td style="text-align:left;font-size:12px;">'.$val['monthly_performance'].'</td>';
    				$strTable .= '<td style="text-align:left;font-size:12px;">'.$team_performance.' </td>';
    				$strTable .= '<td style="text-align:left;font-size:12px;">'.$val['first_leader'].' </td>';
                    $strTable .= '<td style="text-align:left;font-size:12px;">'.date('Y-m-d H:i:s',$val['reg_time']).'</td>';
    				$strTable .= '</tr>';
    			}
    			unset($userList);
    		}
    ...
    public function export_withdrawals(){
        $where=session('tx_where');
        $strTable ='<table width="500" border="1">';
        $strTable .= '<tr>';
        $strTable .= '<td style="text-align:center;font-size:12px;width:120px;">用户ID</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;" width="100">用户昵称</td>';
        ...
        $strTable .= '<td style="text-align:center;font-size:12px;" width="*">银行机构名称</td>';
        ...
        $strTable .= '<td style="text-align:center;font-size:12px;" width="*">提现手续费</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;" width="*">状态</td>';
        $strTable .= '</tr>';
        $count = M('withdrawals')->alias('w')->field('w.*,u.nickname')->join('__USERS__ u', 'u.user_id = w.user_id', 'left')->where($where)->count();
        $p = ceil($count/5000);
        $level=['0'=>'待审核','1'=>'提现成功','2'=>'提现失败'];
        for($i=0;$i<$p;$i++){
            $start = $i*5000;
            $end = ($i+1)*5000;
            $userList =Db::name('withdrawals')->alias('w')->field('w.*,u.nickname')->join('__USERS__ u', 'u.user_id = w.user_id', 'left')->where($where)->order("w.id desc")->limit($start.','.$end)->select();
            if(is_array($userList)){
                foreach($userList as $k=>$val){
                    $strTable .= '<tr>';
                    $strTable .= '<td style="text-align:center;font-size:12px;">'.$val['user_id'].'</td>';
                    ...
                    $strTable .= '<td style="text-align:left;font-size:12px;vnd.ms-excel.numberformat:@">'.$val['bank_card'].'</td>';
                    ...
                    $strTable .= '<td style="text-align:left;font-size:12px;">'.$level[$val['status']].'</td>';
                    $strTable .= '</tr>';
                }
                unset($userList);
            }
        ...
        $strTable .='</table>';
        downloadExcel($strTable,'tixian_'.$i);
        exit();
    }
    //寄售导出
    public function export_consignment(){
        $where=session('js_where');
        $strTable ='<table width="500" border="1">';
        $strTable .= '<tr>';
        $strTable .= '<td style="text-align:center;font-size:12px;" width="*">编号</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;" width="*">账号</td>';
        ...
        $strTable .= '<td style="text-align:center;font-size:12px;" width="*">寄售套餐</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;" width="*">寄售数量</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;" width="*">出售数量</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;" width="*">生成时间</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;" width="*">最后出售时间</td>';
        $strTable .= '</tr>';
        ...
        for($i=0;$i<$p;$i++){
            ...
            $end = ($i+1)*5000;
            // 进行分页数据查询
            $userList = Db::name('goods_consignment')
                ->alias('g')
            ...
            if(is_array($userList)){
                foreach($userList as $k1=>$val){
                    $setmeal = Db::name('goods_setmeal')->where('goods_id', $val['goods_id'])->select();
                    for ($j = 0; $j < 5; $j++) {
                        if ($setmeal[$j]['id'] == $val['setmeal_id']) {
                            $name = '套餐' . ($j + 1);
                            break;
                        }
                    }

                    $num=$val['nums']-$val['surpl'];
                    $strTable .= '<tr>';
                    ...
                    $strTable .= '<td style="text-align:left;font-size:12px;">'.$name.'</td>';
                    $strTable .= '<td style="text-align:left;font-size:12px;">'.$val['nums'].'</td>';
                    $strTable .= '<td style="text-align:left;font-size:12px;">'.$num.'</td>';
