...
class Order extends Base {
    ...
    public function ajaxindex(){
        ...
        foreach ($orderList as $key => $value) {
            $orderList[$key]['mobile2']=M('users')->where('user_id',$value['user_id'])->value('mobile');
        }
...
//        $this->assign('split',$split);
    ...
    public function delivery_infos(){
        $order_id = I('order_id');
        $order = Db::name('self_mention_order')->alias('o')->join('goods g','o.goods_id = g.goods_id')->where("mention_id = $order_id")->find();
//        $orderLogic = new OrderLogic();
//        $order = $orderLogic->getOrderInfo($order_id);
//        $orderGoods = $orderLogic->getOrderGoods($order_id,2);
//        if(!$orderGoods)$this->error('此订单商品已完成退货或换货');//已经完成售后的不能再发货
    ...
    public function export_order()
    {
	    ...
	    if(is_array($orderList)){
	    	...
	    	foreach($orderList as $k=>$val){
                $username=M('users')->where('user_id',$val['user_id'])->value('mobile');
	    		$strTable .= '<tr>';
...
//        echo Db::name('repair_order')->getLastSql();exit;
    ...
    {
        $data = Db::name('self_mention_order')->select();
