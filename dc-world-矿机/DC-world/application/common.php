 ...
 * @return array or string or bool
 */
function worldCa($config_key, $data = array())
{
    $param = explode('.', $config_key);
    if (empty($data)) {
        //如$config_key=shop_info则获取网站信息数组
        ...
        if (empty($config)) {
            //缓存文件不存在就读取数据库
            $res = D('world_system')->where("inc_type", $param[0])->select();
            if ($res) {
                foreach ($res as $k => $val) {
                    $config[$val['name']] = $val['value'];
                }
        ...
        //更新缓存
        $result = D('world_system')->where("inc_type", $param[0])->select();
        if ($result) {
            foreach ($result as $val) {
                $temp[$val['name']] = $val['value'];
            }
            foreach ($data as $k => $v) {
                $newArr = array('name' => $k, 'value' => trim($v), 'inc_type' => $param[0]);
                if (!isset($temp[$k])) {
                    M('world_system')->add($newArr);//新key数据插入数据库
                } else {
                    if ($v != $temp[$k])
                        M('world_system')->where("name", $k)->save($newArr);//缓存key存在且值有变更新此项
                }
            }
            //更新后的数据库记录
            $newRes = D('world_system')->where("inc_type", $param[0])->select();
            foreach ($newRes as $rs) {
                $newData[$rs['name']] = $rs['value'];
            }
        } else {
            ...
            M('world_system')->insertAll($newArr);
            $newData = $data;
        }
        return F($param[0], $newData, TEMP_PATH);
    }
 ...
 * 记录帐户Dc币变动
 * @param   int $user_id 用户id
 * @param   float $number 可用DC币变动
 * @param   string $desc 变动说明
 * @param   int $order_id 订单id type为3是加速的那台矿机购买时的order_id
 * @param   string $order_sn 订单sn type为3是加速的那台矿机购买时的order_sn
 * @param   int $type 类型
 * @param   int $act 是否更新DC币
 * @param   int $union_id type为3时下线下单的order_id  加速释放专用字段
 * @return  bool
 */
function accountPowerLog($user_id, $number = 0, $desc = '', $order_id = 0, $order_sn = '',$type = 0, $act = 0,$union_id = 0)
{
    $before = Db::name('member')->where(['id' => $user_id])->value('powers');
    $after = $before + $number;
    /* 插入帐户变动记录 */
    $account_log = array(
        'user_id' => $user_id,
    ...
    if ($act){
        $update_data = array(
            'powers' => ['exp', 'powers+' . $number],
        );
        $update = Db::name('member')->where('id', $user_id)->update($update_data);
    }
    $res = M('world_power_log')->add($account_log);
    if ($res) {
        return true;
    } else {
        return false;
 ...
 * @return  bool
 */
function accountDcLog($user_id, $number = 0, $desc = '', $order_id = 0, $order_sn = '',$type = 0, $act = 0,$union_id = 0)
{
    $before = Db::name('member')->where(['id' => $user_id])->value('dc_coin');
    $after = $before + $number;
    /* 插入帐户变动记录 */
    $account_log = array(
        'user_id' => $user_id,
...
//        " pay_points = pay_points + $pay_points, distribut_money = distribut_money + $distribut_money WHERE user_id = $user_id";
    if ($act){
        $update_data = array(
            'dc_coin' => ['exp', 'dc_coin+' . $number],
        );
        if ($number == 0)
            return false;
        $update = Db::name('member')->where('id', $user_id)->update($update_data);
    }
    $update = M('world_dc_log')->add($account_log);
    if ($update) {
        return true;
    } else {
        return false;
 ...
 * @return  bool
 */
function accountShellLog($user_id, $number = 0, $desc = '', $order_id = 0, $order_sn = '',$type = 0, $act = 0)
{
    $before = Db::name('member')->where(['id' => $user_id])->value('shell');
    $after = $before + $number;
    /* 插入帐户变动记录 */
    $account_log = array(
        'user_id' => $user_id,
...
//        " pay_points = pay_points + $pay_points, distribut_money = distribut_money + $distribut_money WHERE user_id = $user_id";
    if ($act){
        $update_data = array(
            'shell' => ['exp', 'shell+' . $number],
        );
        if ($number == 0)
            return false;
        $update = Db::name('member')->where('id', $user_id)->update($update_data);
    }
    $update = M('world_shell_log')->add($account_log);
    if ($update) {
        return true;
    } else {
        return false;
 ...
 * 生成钱包地址
 */
function wallet_address_generation($phone,$randLength=18){
    //随机数模板
    ...
    //拼接钱包地址
    $wallet_address = 'DC' . $randStr . substr($phone,-4);

    //查找是否存在
    $result = Db::name('member')->where(array('wallet'=>$wallet_address))->find();

    if($result){
        return wallet_address_generation($phone);
    }
...
{   
   $pattern = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ';
    ...
    $res= M('member')->where(array('invitecode'=>$key))->find();
    if ($res) {
        randomkeys($length);
    } else {
        return $key;
 ...
 * 生成随机助记词
 */
function create_memorizing_word($length=12){
    //随机数模板
    $zhwords = C('ZH_WORDS');
    $words="";
    for ($i = 0; $i < $length; $i++) {
        $words.= iconv_substr($zhwords, floor(mt_rand(0, mb_strlen($zhwords, 'utf-8') - 1)), 1, 'utf-8');
    }
    //查找是否存在
    $result = Db::name('member')->where(array('memorizing_word'=>$words))->find();
    if($result){
        return wallet_address_generation();
    }
    return $words;
}
...
/**
 * 记录日志    2018-11
 * @param $type 变动类型、$content 内容
 */
function apilog($content,$status,$type=0)
{
    $destination = ROOT_PATH. 'runtime'.DS.'log' . DS . date("Ym") . DS . date('d') . 'api.log';
    ...
    $file = fwrite($f, print_r($message . '[ 内容 ] ', true));
...
function base64_image_content($base64_image_content,$path)
{
    ...
    if (preg_match('/^(data:\s*image\/(\w+);base64,)/', $base64_image_content, $result)) {
        ...
        $new_file = $new_file . date('tmd').time() . ".{$type}";
 ...
 * @throws \think\Exception
 */
function message_notificat($user_id,$type,$title,$content,$union_id=0){
    $create_time = time();
    ...
    $result = Db::name('user_notice')->add($data);
 ...
 * 标位已读
 */
function sign_read($id,$user_id){
        $res = M('user_notice')->where(array('id'=>$id))->update(array('is_read'=>1));
        //标位已提示
        $where =array(
            'user_id'=>$user_id,
            'is_tips'=>0,
