...
class DistributPrizeLogic extends Model
{
    ...
    public function recommended_prize(){
        ...
        try {
            ...
            $this->recommendedDistribut($this->user['first_leader'], 'first_leader');
            $this->recommendedDistribut($this->user['second_leader'], 'second_leader');
    ...
    private function recommendedDistribut($user_id, $level){
        // 获取推荐人信息
    ...
    public function management_prize(){
        ...
        $mana_users = Db::name('users')
            ...
            ->where(['level'=> ['in',$mana_id_list],'first_leader'=>['GT',0]])
            ->select();
        $Distribut = new DistributPrize();
        foreach ($mana_users as $key => $val)
        {
            ...
            if ($val['level'] == 3){
                if ($first_leader_level === $seconed_leader_level){
                    $this->managementDo($val, $config, 'third');
                    continue;
                }
                $arr = [4,5,6];
                if (in_array($first_leader_level, $arr)){
                    $this->managementDo($val, $config, 'second');
                    continue;
                }
            }
     ...
     * 获取自身  上级  当日
     * */
    public function managementDo($user, $config,$level){
        try {
    ...
    public function region_city_prize(){
        ...
        foreach ($city_users as $key => $val)
        {
            ...
            $this->regionPrizeDo($val,$orderMoney,$last_partner_prize,$money_unit_config);
        }
    }
     ...
     * 处理分销奖
     * */
    public function regionPrizeDo($userInfo, $orderMoney, $last_partner_prize, $config){
