...
class Test extends Controller
{
    ...
    {
        $user = M('users')->where(array('user_id' => array('in', array(21, 22, 23))))->select();
//        dump($user);

        $auth_users = M('oauth_users')->select();
//        $auth_users =  M('oauth_users')->where(array('openid'=>'oATf755J-NAXhx8thvfP-VUD9GbM'))->delete();

//        dump($auth_users);

        $ad_position = M('ad_position')->where(array('position_id' => array('not in', array(32, 51318, 51319, 51320, 51301))))->delete();
//        dump($ad_position);
        $pdata = input('');
        dump($pdata);
    ...
    public function wxrefund()
    {
        ...
        $order = $orderLogic->getOrderInfo(14);
