...
class DistributLogic extends Model{
    ...
    public function rebateLog($order){
        ...
        //寻找地区合伙人
        $partner_district = $order['city'];

        $partner = M('users')->field('user_id')->where(array('agent_city'=>$partner_district,'level'=>3))->find();
        ...
        try{
            ...
            $this->userDistributDo($partner['user_id'],4,$order);
    ...
    {
        $GLOBALS['data'] = tpCache('distribut.date');//防止冲突


        //由于分佣分成不知道，不确定有多少条数，只能通过数据分批处理
        M('rebate_log')->where(['user_id'=>$user_id,'status'=>array('lt',3)])->chunk(20,function($user){
            $user_logic = new UsersLogic();
            $id_arr = [] ; //需更新的id

            foreach($user as $log)
            {
                $state_time =  $log['can_distribution'] == 1 ? 'pay_time' : 'confirm';
                if( (time() - $log[$state_time]) >= $GLOBALS['data'] * 86400 && $log['status'] >= $log['can_distribution']){
                    if($log['type'] == 1){$type = '区域奖';}else{$type = '分销奖';}
                    $rebate_log = ['desc'=>'分佣获得余额-'.$type,'order_sn'=>$log['order_sn'],'order_id'=>$log['order_id']];
    ...
    private function userDistributDo($user_id,$level,$order)
    {
        ...
        switch($level)
        {
            ...
            case 3:
                ...
                break;
            case 4:
                $price = $distribut_price *  tpCache('distribut.region_rate');
                break;
        }
        ...
        $data['can_distribution'] = tpCache('distribut.can_distribution');
        $data['pay_time'] = time();
