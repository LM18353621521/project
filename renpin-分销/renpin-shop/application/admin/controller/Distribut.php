...
//        $lists = M('rebate_log')->order("field(status,2,1,0,3,4)")->where($where)->limit($Page->firstRow . ',' . $Page->listRows)->select();
    ...
    public function yesurl(){
        ...
        if(!$rebate[$state_time] || (time() - $rebate[$state_time]) <= tpCache('distribut.date') * 86400)
            $this->error($affirm_msg.'，未达到规定时间进行确认分成。'.tpCache('distribut.date').'天');
        ...
        if($rebate['type'] == 1){$type = '区域奖';}else{$type = '分销奖';}
        $rebate_log = ['desc'=>'分佣获得余额-'.$type,'order_sn'=>$rebate['order_sn'],'order_id'=>$rebate['order_id']];
