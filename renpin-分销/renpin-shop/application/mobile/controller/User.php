    ...
    */
    public function _initialize()
    {
        ...
        $nologin = array(
            ...
            'forget_pwd', 'check_captcha', 'check_username', 'send_validate_code', 'express' , 'bind_guide', 'bind_account','contactleader'
        );
        ...
        $user = session('user');
        if($user && !in_array(ACTION_NAME, $nologin)){
...
//            }
    ...
    public function my_qrcode(){
        ...
        if(!$userId){
            ...
            //获取个人
            $url = request()->domain().U('contactleader',['id'=>$userId]);
        }
        if(!$user['short_url']){
        ...
        $this->assign('qrcode_path',request()->domain().'/'.$after_path);
        $this->assign('user',$user);
        return $this->fetch();
    }
